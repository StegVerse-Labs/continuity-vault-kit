...
material for one operation, or preferably a TVC-resident key provider callback; SKAP
derives an object/version-specific AES-256-GCM key with HKDF-SHA256, authenticates
immutable context as AAD, and returns only a sealed ciphertext envelope. Resolution
is callback-only: plaintext is not returned and a mutable copy is overwritten
immediately after the callback completes.
...
of mutable key/plaintext buffers under its control.
"""
...
def _derive_key(root_key: bytes | bytearray | memoryview, salt: bytes, aad: bytes) -> bytes:
    if not isinstance(root_key, (bytes, bytearray, memoryview)) or len(root_key) < 32:
...
def seal_with_provider(
    ...
    *,
    ...
    """Seal without exposing raw root-key bytes to the caller API."""
    return key_provider.with_key(lambda key: seal(
        plaintext,
        root_key=key,
        object_id=object_id,
        credential_version=credential_version,
...
def resolve_transiently(
    ...
    *,
    ...
    expected = _aad_context(object_id=expected_object_id, credential_version=expected_credential_version, wrapping_policy_ref=expected_wrapping_policy_ref, purpose=expected_purpose, endpoint_ref=expected_endpoint_ref)
...
def resolve_granted_with_provider(
    sealed: dict[str, Any],
    *,
    key_provider: KeyProvider,
    lifecycle_state: str,
    ...
    """Resolve through TVC key-provider authority without returning root-key bytes."""
    return key_provider.with_key(lambda key: resolve_granted_transiently(
        sealed,
        root_key=key,
        lifecycle_state=lifecycle_state,
        current_credential_version=current_credential_version,
        grant=grant,
        revocation_check_passed=revocation_check_passed,
        expected_object_id=expected_object_id,
        expected_wrapping_policy_ref=expected_wrapping_policy_ref,
        expected_key_authority_ref=key_provider.authority_ref,
        consumer=consumer,
    ))
